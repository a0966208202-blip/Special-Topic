...
(訓練程式 - 原始資料 + Augmentation + EarlyStopping)
(*** Gemini 修正版 v2：加入 Dropout 和 Class Weights ***)
"""
...
output_dir = r"C:\Users\USER\Desktop\CT_PRO\實驗_原始資料訓練\result_v2_Dropout_ClassWeight" # <-- 建議改個新資料夾

# ---
# ✅ (2/3) 定義新指標圖片的儲存資料夾
plot_save_dir = r"C:\Users\USER\Desktop\CT_PRO\實驗_原始資料訓練\新增資料夾_v2" # <-- 建議改個新資料夾
# ---
...
train_datagen = ImageDataGenerator(
    ...
    brightness_range=[0.9, 1.1] # 隨機調整亮度 (90%~110%)
)
...
# === ✅ (B) 計算 Class Weights (用來解決類別不平衡) ===
# 獲取訓練集中的標籤 (0, 1, 2...)
train_labels_int = train_generator.classes 
...
# 將權重轉為 Keras 需要的字典格式
# train_generator.class_indices 會告訴你 'Hemorrhagic': 0, 'Ischemia': 1, 'Normal': 2
class_weights_dict = dict(zip(np.unique(train_labels_int), class_weights))

print("\n--- Class Weights (For Imbalance) ---")
print(f"Class Indices: {train_generator.class_indices}")
print(f"Weights Dictionary: {class_weights_dict}")
print("---------------------------------------")


# === EfficientNet Model (✅ 加入 Dropout) ===
...
# ModelCheckpoint (建議改為監控 val_loss)
checkpoint_callback = ModelCheckpoint(
    filepath=checkpoint_filepath,
    save_best_only=True,
    monitor="val_loss",  # <-- 改為監控 val_loss
    mode="min",          # <-- 模式改為 'min'
    verbose=1
)
...
early_stopping_callback = EarlyStopping(
    ...
    patience=10,             # 如果 10 個 epochs 內 val_loss 都沒有降低，就停止
    verbose=1,
    mode="min",
    restore_best_weights=True  # 停止時，自動恢復到最佳權重
)

print("\n--- Starting Fine-tuning (with Augmentation, Early Stopping, Dropout, Class Weights) ---")
# ---
# ✅ 3. 將 EarlyStopping 和 Class Weights 加入 model.fit
# ---
history_finetune = model.fit(
    train_generator,
    validation_data=val_generator,
    epochs=100, # 可以設一個較大的數字，EarlyStopping 會自動處理
    callbacks=[checkpoint_callback, early_stopping_callback], # <-- 加入 Callbacks
    class_weight=class_weights_dict  # <--- ✅ 加入 Class Weights
)
print("\n--- Fine-tuning Finished ---")

# 檢查模型是否因為 EarlyStopping 而提早停止
if early_stopping_callback.stopped_epoch > 0:
    print(f"✅ Training stopped early at epoch {early_stopping_callback.stopped_epoch + 1} to prevent overfitting.")
...
# 因為 restore_best_weights=True，model 物件本身就是最好的模型
# 但為了保險起見，我們仍然可以從檔案載入 (兩者在此情況下應相同)
